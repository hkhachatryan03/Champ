...
export default function CroppablePhotoInput({ name }: { name: string }) {
    ...
    <div>
      ...
      {previewUrl ? (
        <div className="flex items-center gap-3">
          ...
          <img src={previewUrl} alt="" className="w-14 h-14 rounded-full object-cover" />
          <button type="button" onClick={() => pickerRef.current?.click()} className="text-xs underline text-apricot-deep">
            Choose a different photo
          </button>
          <button type="button" onClick={clearPhoto} className="text-xs underline text-muted">
            Remove
          </button>
        </div>
      ) : (
        <button
          ...
          className="px-4 py-2 rounded-lg text-sm font-medium bg-apricot text-ink"
        >
          Choose photo
        </button>
      )}
